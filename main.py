...
train_set, test_set, train_cls_freq, test_cls_freq, stopwords = train.extract_dataset(dataset_path,
                                                                                      stop_words_path,
                                                                                      data_filter,
                                                                                      train_key, test_Key,
                                                                                      classes_col)
...
train_unique_vocabulary, train_vocabulary_freq = train.clean_tokenize_freq_dist(train_set, vocabulary_col, True)
...
train_cls_word_freq, train_cls_word_prob, train_cls_prob, training_data, excluded_vocab = train.generate_model(train_unique_vocabulary,
                                                                                               train_set,
                                                                                               classes_col,
                                                                                               vocabulary_col,
                                                                                               train_cls_freq,
...
train.store_dataframe_to_file(training_data, csv_path=csv_path, text_path=text_path)
"""Store vocabulary data frame to file"""
train.store_dataframe_to_file({"vocabulary": list(train_unique_vocabulary)}, csv_path=None, text_path=vocabulary_path)
"""Store excluded data frame to file"""
train.store_dataframe_to_file({"removed": [str(i).encode('utf-8') for i in excluded_vocab]}, csv_path=None, text_path=removed_word_path)
